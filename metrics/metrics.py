...
def analyze_text(text_name, list_csv):
    ...
    final_list_csv_scores.insert(0, text_name)
...
if __name__ == "__main__":
    text_names = sys.argv[1]
    text_names = text_names.split(" ")
    list_csv = []

    for text in text_names:
        (_, list_csv) = analyze_text(text, list_csv)

    cols = ['poem_name', 
        'ttr_orig', 'tok_sent_orig', 'char_word_orig', 'ttr_gen', 'tok_sent_gen', 'char_word_gen', 'ttr_ppt', 'tok_sent_ppt', 'char_word_ppt', 
    ...
    df_texts = pd.DataFrame(data=list_csv, columns=cols)
    df_texts.to_csv('text_metrics.csv', index=False)
